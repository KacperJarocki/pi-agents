...
| 7 | `frame.len` | ✅ | Rozmiar ramki → `bytes_sent` |
...
- Jeśli `src_ip` jest w LAN → to urządzenie lokalne wysyła → użyj `eth.src`
- Jeśli `dst_ip` jest w LAN → to ruch przychodzi do urządzenia → użyj `eth.dst`
...
| `bytes_received` | Zawsze 0 (collector nie rozróżnia) |
