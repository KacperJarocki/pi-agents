...
```bash
./scripts/model-backtest.sh \
  --database-path /data/iot-security.db \
  --model-root /data/models \
  --device-id 10 \
  --model-type isolation_forest \
  --start "2026-05-19 19:20:00" \
  --end "2026-05-19 19:40:00" \
  --label attack_port_sweep
```

Jeśli chcesz sprawdzić konkretną archived wersję modelu, użyj `--model-registry-id <id>`. Jeśli chcesz użyć pliku bez registry, użyj `--model-file /data/models/archive/.../model.joblib`.
...
- `Historical Model Replay` — wykres `risk_score` wybranego modelu na historycznych bucketach już zapisanych przez inference (`device_model_scores`).
- `Model Versions` — lista archived modeli z przyciskiem `Activate` do rollbacku aktywnego artefaktu.

Pełne przeliczenie archived modelu na wybranym oknie danych jest nadal operacją CLI (`model-backtest.sh`), ponieważ wymaga zależności ML (`sklearn`, `pandas`, `joblib`) z obrazu `ml-pipeline`.
