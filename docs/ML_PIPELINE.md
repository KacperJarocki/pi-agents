...
Porównanie czterech aktualnych artifactów na tym samym oknie:
...
Dashboard używa endpointu `GET /api/v1/devices/{id}/model-replay`. Tryb `model_type=all` jest zoptymalizowany pod UI: gateway-api czyta flow i liczy feature buckets tylko raz, a potem przepuszcza te same buckety przez IF/LOF/OCSVM/Autoencoder. CLI (`model-backtest.sh`) nadal jest wygodniejszy do długich, opisanych eksperymentów z `summary.json` i TP/FP/FN/TN.
