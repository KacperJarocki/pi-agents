...
class TrafficCollector:
    ...
    async def _stop_process(self):
        ...
        if self._process.returncode is None:
            ...
            try:
                ...
                log.info("capture_process_stopped", pid=self._process.pid, returncode=self._process.returncode)
            except TimeoutError:
                log.warning("capture_process_kill", pid=self._process.pid)
                self._process.kill()
                await self._process.wait()
                log.warning("capture_process_killed", pid=self._process.pid, returncode=self._process.returncode)
    ...
    async def _capture_loop(self):
        while self.running:
            ...
            try:
                cmd = build_tcpdump_command(self.interface, pcap_file, self.capture_packet_count)
                ...
                timed_out = False
                try:
                    await asyncio.wait_for(self._process.wait(), timeout=self.capture_timeout)
                except TimeoutError:
                    timed_out = True
                    await self._stop_process()
    ...
    async def _process_pcap(self, pcap_file: str, cycle: int):
        ...
        try:
            ...
            lines = [line for line in stdout.decode(errors="replace").split('\n') if line]
            log.info(
                "tshark_finished",
                cycle=cycle,
            ...
            log.error("pcap_processing_error", error=str(e), cycle=cycle, path=pcap_file)
    ...
    async def _flush_buffer(self):
        ...
        try:
            log.info("flush_started", requested=len(flows_to_process), remaining_buffer=len(self.flow_buffer))
