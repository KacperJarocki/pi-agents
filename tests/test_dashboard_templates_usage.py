...
class TestDashboardTemplateUsage(unittest.TestCase):
    def test_template_response_uses_request_first(self):
        ...
        self.assertIn("Primary / Shadow Research", device_template)
        self.assertIn("Only the primary model affects live risk", device_template)
        self.assertIn("model-research-summary", device_template)
        self.assertIn("renderModelResearchSummary", device_template)
