...
class TestMlMvpSources(unittest.TestCase):
    ...
    def test_k8s_ml_manifests_set_mvp_env(self):
        ...
        self.assertIn('ADAPTIVE_CONTAMINATION_MIN', trainer)
    ...
    def test_inference_builds_behavior_alerts(self):
        ...
        self.assertIn("dns_nxdomain_burst", inference)
        self.assertIn("icmp_sweep_suspected", inference)
        self.assertIn("icmp_echo_fanout", inference)
        self.assertIn("def _baseline_stats(values: list[float])", inference)
        self.assertIn("def _risk_with_contributors(ml_risk: float, behavior_alerts: list[dict])", inference)
